...
SparseElement *transposeSparseMatrix(SparseElement *matrix)
{
    SparseElement *transpose = NULL;
    SparseElement *current = matrix;

    while (current != NULL)
    {
        addSparseElement(&transpose, current->col, current->row, current->value);
        current = current->next;
    }
...
int main()
{
    ...
    SparseElement *transpose = transposeSparseMatrix(matrix);
    printf("Transpose of Sparse Matrix:\n");
    displaySparseMatrix(transpose, cols, rows);
